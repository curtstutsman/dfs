...
COMMON_OBJS = $(OBJ)/Log.o
CLIENT_OBJS = $(OBJ)/ClientBase.o $(OBJ)/ClientNode.o $(OBJ)/Client.o
SERVER_OBJS = $(OBJ)/ServiceImpl.o $(OBJ)/ServerNode.o

vpath %.cpp src/common src/client src/server
vpath %.cc  proto-src
...
clean:
	rm -f $(BIN)/dfs-client $(BIN)/dfs-server $(OBJ)/*.o

clean_all: clean
	rm -f proto-src/*.pb.cc proto-src/*.pb.h

.PHONY: all protos dirs clean clean_all
